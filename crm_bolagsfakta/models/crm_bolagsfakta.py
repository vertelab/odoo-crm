...
class CRMBolagsfakta(models.Model):
    ...
    number_of_result = fields.Integer(string="Number of Result", default=50)
    turn_over = fields.Float(string="Turnover")
    number_of_employees = fields.Integer(string="Number of Employees", default=1)
        ...
        for municipality in self.municipality_ids:
            ...
            if self._has_over_1000(soup=soup):
                ...
                companies_data = extract_companies(
                    ...
                    number_of_result=self.number_of_result
                )
    ...
    def process_sub_categories(self, soup, municipality):
        ...
        for sub_category_link in sub_categories_links:
            ...
            if 0 < self.number_of_result <= len(companies_data):
                break

            sub_response = requests.get(sub_category_link)
            sub_soup = BeautifulSoup(sub_response.content, 'html.parser')

            remaining_results = self.number_of_result - len(companies_data) if self.number_of_result > 0 else 0
            sub_companies = extract_companies(
                soup=sub_soup,
                municipality=municipality,
                crm_data=self._base_metadata(),
                filter_params=self._filter_params(),
            ...
            companies_data.extend(sub_companies)
