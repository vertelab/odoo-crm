...
def extract_companies(soup, municipality, crm_data, filter_params, number_of_result=0):
    """
    Extract company information from the soup object with result limit handling.

    Args:
        soup: BeautifulSoup object containing the page HTML
        municipality: Municipality object
        crm_data: Dictionary containing CRM-related data
        filter_params: Dictionary containing filter parameters
        number_of_result: Integer specifying max number of results (0 means no limit)
    ...
    for box in company_boxes:
        ...
        if 0 < number_of_result <= len(companies_data):
        ...
        if inner_box:
            ...
            if is_valid:
                ...
                # Check again after adding a new company
                if 0 < number_of_result <= len(companies_data):
                    return companies_data

    # Check for pagination only if we haven't reached the desired number of results
    if number_of_result == 0 or len(companies_data) < number_of_result:
        ...
        if pagination:
            ...
            if next_page_link:
                ...
                remaining_results = number_of_result - len(companies_data) if number_of_result > 0 else 0
                companies_data.extend(extract_companies(
                    soup=next_page_soup,
                    municipality=municipality,
                    crm_data=crm_data,
                    filter_params=filter_params,
                    number_of_result=remaining_results
                ))
